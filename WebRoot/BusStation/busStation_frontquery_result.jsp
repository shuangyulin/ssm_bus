...
<div id="busStationEditDialog" class="modal fade" tabindex="-1" role="dialog">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
        <h4 class="modal-title"><i class="fa fa-edit"></i>&nbsp;站点信息信息编辑</h4>
      </div>
      ...
      <div class="modal-footer"> 
      	...
      	<button type="button" class="btn btn-primary" onclick="ajaxBusStationModify();">提交</button>
      </div>
    </div><!-- /.modal-content -->
  </div><!-- /.modal-dialog -->
</div><!-- /.modal -->
	...
	$.ajax({
		...
		success : function (busStation, response, status) {
			if (busStation) {
				...
				$('#busStationEditDialog').modal('show');
			} else {
				alert("获取信息失败！");
			}
		}
...
/*删除站点信息信息*/
function busStationDelete(stationId) {
	if(confirm("确认删除这个记录")) {
		$.ajax({
			type : "POST",
			url : basePath + "BusStation/deletes",
			data : {
			...
			success : function (obj) {
				if (obj.success) {
					...
					//location.href= basePath + "BusStation/frontlist";
				}
				else 
					alert(obj.message);
			},
